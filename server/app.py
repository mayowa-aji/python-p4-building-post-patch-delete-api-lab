...
@app.route('/bakeries/<int:id>', methods=['GET', 'POST', 'PATCH'])
def bakery_by_id(id):
...
def baked_goods():
    if request.method == 'GET':
        ...
        baked_goods=[]
        for baked_good in BakedGood:
            baked_good.append(baked_goods.to_dict())
        response = make_response(
            baked_goods,
            200
        )
    ...
    def baked_goods_by_id(id):
        baked_good = BakedGood.query.filter.by(id=id).first()
